...
int ip_rx(struct subuff *sub)
{
    ...
    switch (ih->proto) {
        case IPP_NUM_ICMP:
            debug_ip_hdr("in", ih);
            debug_ip("[M2] incoming ICMP packet, further ICMP packet processing is not yet implemented \n");
            icmp_rx(sub);
            return 0;
        case IPP_TCP:
            debug_ip_hdr("in", ih);
            debug_ip("[M3] incoming TCP packet, further TCP packet processing is not yet implemented\n");
            tcp_rx(sub);
            return 0;
        default:
